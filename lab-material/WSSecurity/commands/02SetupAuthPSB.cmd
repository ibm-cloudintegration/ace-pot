...
mqsireportproperties %nodeP% -c PolicySets -o myProviderPolicySet -r > %Reps%AuthOnly%nodeP%PS.txt

@echo reporting PROVIDER Policy Set Binding
mqsireportproperties %nodeP% -c PolicySetBindings -o myProviderPolicySetBinding -r > %Reps%AuthOnly%nodeP%PSBinding.txt
...
mqsireportproperties %nodeC% -c PolicySets -o myConsumerPolicySet -r > %Reps%AuthOnly%nodeC%PS.txt

@echo reporting          Policy Set Binding
mqsireportproperties %nodeC% -c PolicySetBindings -o myConsumerPolicySetBinding -r > %Reps%AuthOnly%nodeC%PSBinding.txt
...
@echo 02SetupAuthPSB command complete.